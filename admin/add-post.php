...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($_FILES['featured_image']['type'], $allowed_types)) {
            ...
            $featured_image = uniqid('blog_') . '.' . $file_extension;
    ...
    // Se non ci sono errori, salva l'articolo
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO blog_posts (title, slug, content, excerpt, featured_image, status, author_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
        $stmt->bind_param("ssssssi", $title, $slug, $content, $excerpt, $featured_image, $status, $author_id);
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = "Errore durante il salvataggio dell'articolo: " . $conn->error;
            
            // Se c'è stato un errore, elimina l'immagine caricata
            if (!empty($featured_image) && file_exists($upload_path)) {
                unlink($upload_path);
            }
        }
    }
...
// Titolo della pagina
$page_title = "Aggiungi Nuovo Articolo";
?>
...
<head>
    ...
    <title>Aggiungi Articolo - Dashboard</title>
                    ...
                    <div class="card-body">
                        <form action="" method="POST" enctype="multipart/form-data" class="form">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select id="status" name="status" class="form-control">
                                        <option value="draft" <?php echo isset($_POST['status']) && $_POST['status'] === 'draft' ? 'selected' : ''; ?>>Bozza</option>
                                        <option value="published" <?php echo isset($_POST['status']) && $_POST['status'] === 'published' ? 'selected' : ''; ?>>Pubblicato</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="slug" name="slug" class="form-control" value="<?php echo htmlspecialchars($_POST['slug'] ?? ''); ?>">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="content" name="content" class="form-control"><?php echo htmlspecialchars($_POST['content'] ?? ''); ?></textarea>
                            </div>
